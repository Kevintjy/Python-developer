...
class LinkedListRec:
    ...
    def __contains__(self, item: object) -> bool:
        ...
        if item == self._first:
            return True
        return any([self._rest.__contains__(item)]) if self._rest is not None else False
    ...
    def map(self, f: Callable[[object], object]) -> 'LinkedListRec':
        ...
        '5 -> 7'
        """
        new_linked = LinkedListRec([])
        if self._first is not None:
            new_linked._first = f(self._first)
            new_linked._rest = self._rest.map(f)
        return new_linked
