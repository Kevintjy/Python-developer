...
class Tree:
    ...
    def count_occurrences(self, value):
        ...
        """
        if self.value == value:
            return 1 + sum([node.count_occurrences(value) for node in self.children])
        return sum([node.count_occurrences(value) for node in self.children])
    ...
    def add_subtree_to(self,tree, value):
        ...
        if self.value == value:
            ...
            return True
        for node in self.children:
            recursive_call = node.add_subtree_to(tree, value)
            if recursive_call:
                return True
        return False

    def get_level_items(self) -> List[List[Any]]:
        level = []
        recursive_call = [child.get_level_items() for child in self.children]
