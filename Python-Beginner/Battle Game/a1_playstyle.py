...
class RandomPlayStyle(Playstyle):
    ...
    def select_attack(self, parameter = random.choice(['A', 'S'])):
        """
        Return the random attack for the character in this Playstyle's
        battle_queue to perform
        """
        if not self.battle_queue.peek().is_valid_action("S"):
            return "A"
